...
export default function ProfileInfo({
  ...
  async function handlFollowClick() {
  ...
  useEffect(() => {
    if (dataFriends && dataFriends.length > 0) {
      dataFriends.map((user) => {
        if (user.nickName === urlname.substring(9)) {
            ...
            <div className="md:w-full md:h-10">
              ...
              <div>
                <div className="md:w-full md:bg-[#4A4953] md:rounded-full md:h-2.5 xl:h-3 ">
                  <div className="md:bg-[#E95A3A] md:h-2.5 xl:h-3 md:rounded-full md:w-[100%]"></div>
                </div>
              </div>
              <p
                className={`${NeuePlakFont.className} text-[9px] lg:text-[11px] xl:text-[14px] 2xl:text-[20px] text-end text-white`}
              >
                {`Level ${dataprofile.level.toFixed(1)}`}
              </p>

...
import { add_admin, add_member, create_channel, join_protected_channel } from "src/utils/types";
...
export class ChatController{
	...
	// delete friendship send a refresh of profile and friends  deselcet friendid

	// add friendship send a refresh of profile and friends  deselcet friendid
	...
	@UseGuards(AuthenticatedGuard)
	@Post('add_member')
	async add_members(@Session() session: Record<string,any>,@Body() body: add_member)
	{
		// add a friend to be a member
		if (!body.name && !body.channel_id)
	...
	async kick(@Session() session: Record<string,any>,@Body() body: add_admin)
	{
		...
		if (!body && !body.member_id && !body.channel_id)
			throw new HttpException("can't kick the member", HttpStatus.FORBIDDEN);
	...
	{
		// send refresh to channels
		if (!data.channel_id || !data.type)
			throw new HttpException("error",HttpStatus.FORBIDDEN);
		if (data.type == RoomType.PROTECTED && !data.password)
			throw new HttpException("error",HttpStatus.FORBIDDEN);
		if (!( await this.chatService.channel_password(data,session.passport.user.id) ))
			throw new HttpException("error",HttpStatus.FORBIDDEN);
	}

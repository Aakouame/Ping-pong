...
import { add_admin, add_member, channels, create_channel, join_protected_channel } from "src/utils/types";
...
export class chatService {
	...
	{
		let b : channels = {id : "", isJoined: false, nameOfChannel : "", type: "" , photo: ""};
	...
	async check_privileges(userid: string, member_name : string, channelid: string)
	{
		try {
	...
	async add_member(userid: string ,channel : add_member)
	{
		if (await this.check_privileges(userid,channel.name,channel.channel_id))
		{
			try {
				const room = await this.prisma.room.update({
					where : {
	...
	async  channel_password(data: update_password,userid: string)
	{
		try {
